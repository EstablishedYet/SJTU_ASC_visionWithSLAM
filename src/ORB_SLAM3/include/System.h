...
public:
    ...
    // Input images: RGB (CV_8UC3) or grayscale (CV_8U). RGB is converted to grayscale.
    // Returns the camera pose (empty if tracking fails).
    Sophus::SE3f TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas = vector<IMU::Point>(), string filename="",bool createNewDelay=false);

    Sophus::SE3f RelocalizeMonocular(const cv::Mat &im,const double &timestamp,string filename,bool &flag);
    ...
    void SaveFrame(int circle=1);
    bool getResetActiveMapFlag();
    int getState();
