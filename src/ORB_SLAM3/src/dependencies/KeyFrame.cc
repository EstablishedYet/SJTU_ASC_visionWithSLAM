...
KeyFrame::KeyFrame():
        ...
        mbf(0), mb(0), mThDepth(0), N(0), mvKeys(static_cast<vector<cv::KeyPoint> >(NULL)), mvKeysUn(static_cast<vector<cv::KeyPoint> >(NULL)),mvKeysUn_nmtx(static_cast<vector<cv::KeyPoint> >(NULL)),
...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mbf(F.mbf), mb(F.mb), mThDepth(F.mThDepth), N(F.N), mvKeys(F.mvKeys), mvKeysUn(F.mvKeysUn), mvKeysUn_nmtx(F.mvKeysUn_nmtx),
...
void KeyFrame::UpdateBestCovisibles()
{
    ...
    mvpOrderedConnectedKeyFrames = vector<KeyFrame*>(lKFs.begin(),lKFs.end()); ///in decrease order
