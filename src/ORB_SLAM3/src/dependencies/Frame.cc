     ...
     mpCamera2(nullptr), mbHasPose(false), mbHasVelocity(false)
{
    ...
    N = mvKeys.size();  ///vector of keypoints, generated by ExtractORB
    if(mvKeys.empty())
        return;
...
bool Frame::PosInGrid(const cv::KeyPoint &kp, int &posX, int &posY)
{
    ...
    //Keypoint's coordinates are undistorted, which could cause to go out of the image  /// it seems to be not possible
