...
void MapPoint::PostLoad(map<long unsigned int, KeyFrame*>& mpKFid, map<long unsigned int, MapPoint*>& mpMPid)
{
    ...
    mpReplaced = static_cast<MapPoint*>(NULL);
    if(mBackupReplacedId>=0)  /// indicate to the real point, which replace this one 
    {
    ...
    mObservations.clear();  ///used to save in which keyFrames the point is detected
    ...
    }  ///the 2 number in tuple are not coordinates, coordinates are saved in KeyPoint in KeyFrame, KeyPoint can be found by the 2 number
