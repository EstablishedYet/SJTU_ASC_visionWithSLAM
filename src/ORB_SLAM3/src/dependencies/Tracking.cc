...
void Tracking::storeFrame(Frame &frame)
{
    unique_lock<mutex> lock(frameStorageMutex);
    // ostream f;
    string storeFile=frame.mNameFile;
    // size_t pos=storeFile.rfind("jpg");
    // storeFile.replace(pos,3,"txt"); ///to do change the change of filename to System::SaveFrame
    // f.open(storeFile);
    if(storeFile!="")
    {
        auto& innerMap=frameStorage[storeFile];
...
void Tracking::storeKeyFrame(KeyFrame *frame)
{
    unique_lock<mutex> lock(frameStorageMutex);

    string storeFile=frame->mNameFile;
    // size_t pos=storeFile.rfind("jpg");
    // storeFile.replace(pos,3,"txt");
    // f.open(storeFile);
    if(storeFile!="")
    {
        auto& innerMap=frameStorage[storeFile];
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename,bool createNewDelay)
{
    mImGray = im;
    if(mImGray.channels()==3)
    {
    ...
    Track(createNewDelay);

    return mCurrentFrame.GetPose();
}

Sophus::SE3f Tracking::GrabImageRelocalize(const cv::Mat &im,const double &timestamp,string filename,bool &flag)
{
    mImGray = im;
    if(mImGray.channels()==3)
    {
...
void Tracking::Track(bool createNewDelay,bool gap)
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            ...
            else
            {
                ...
                else if (mState == LOST) ///the branch happen only when pCurrentMap->KeyFramesInMap()<10
                {
        ...
        else
        {
            ...
            if(mState==LOST) ///but in tracking mode mState will never be Lost
            {
                if(mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD)
            ...
            else
            {
                ...
                else
                {
                    ...
                    {
                        mbVO = false;
                    }///todo if both fail,which one is better??? and if both fail then what to do??? should you turn recent lost into lost and complement relative logic in summer2025.cc???
        ...
        else
        {
            ...
            if(bOK && !mbVO) ///apparently that the author want to tracklocalmap if relocalize successfully
                bOK = TrackLocalMap();
        }
        ...
        if(bOK || mState==RECENTLY_LOST) ///attention to the velocity in relocalization!!!!
        {
            // Update motion model
            if(mLastFrame.isSet() && mCurrentFrame.isSet() && !gap)
            {
                Sophus::SE3f LastTwc = mLastFrame.GetPose().inverse();
                mVelocity = mCurrentFrame.GetPose() * LastTwc;
                mbVelocity = true;  ///may have to ban velocity when repeatively relocalize, also pay attention to the choose of referenceKF
            }///如果回环后第一张就relocalize成功，则应ban掉，否则不需？错！ 都ban掉  ||| referencekf会自动更新，lastframe照旧不影响
            else {
                mbVelocity = false;
            }
            ...
            if(bNeedKF && (bOK || (mInsertKFsLost && mState==RECENTLY_LOST &&
                                   (mSensor == System::IMU_MONOCULAR || mSensor == System::IMU_STEREO || mSensor == System::IMU_RGBD))))
                newFrameCreated=CreateNewKeyFrame();
            ...
            if(newFrameCreated||mState==RECENTLY_LOST)
            {
                for(int i=0; i<mCurrentFrame.N;i++)
                {
                    if(mCurrentFrame.mvpMapPoints[i] && mCurrentFrame.mvbOutlier[i])
            ...
            else
            {
                ...
                string storeFile=mCurrentFrame.mNameFile;
                if(storeFile=="")
                {
                    for(int i=0; i<mCurrentFrame.N;i++)
                    {
                        if(mCurrentFrame.mvpMapPoints[i])
                        {
                            if(mCurrentFrame.mvbOutlier[i])
                                mCurrentFrame.mvpMapPoints[i]=static_cast<MapPoint*>(NULL);
                            // else
                            //     innerMap[mCurrentFrame.mvpMapPoints[i]]=Coordinate(mCurrentFrame.mvKeysUn_nmtx[i].pt.x,mCurrentFrame.mvKeysUn_nmtx[i].pt.y);
                        }
                    }
                }
                else
    ...
    if(mState==OK || mState==RECENTLY_LOST)  ///can consider delete them to enhance speed
    {
...
void Tracking::CreateInitialMapMonocular()
{
    ...
    for(size_t i=0; i<mvIniMatches.size();i++)
    {
        ...
        MapPoint* pMP = new MapPoint(worldPos,pKFcur,mpAtlas->GetCurrentMap()); ///then save mappoints for 1st and 2nd frame, other frames at elsewhere
    ...
    initID = pKFcur->mnId;
    storeFrame(mInitialFrame);
    storeFrame(mCurrentFrame);
}
...
void Tracking::UpdateLocalKeyFrames()
{
    ...
    if(pKFmax)
    {
        mpReferenceKF = pKFmax; ///mpReferenceKF would reset everytime a keyframe created or everytime tracklocalmap
        mCurrentFrame.mpReferenceKF = mpReferenceKF;   ///确认一下若relocalization fail，referenceKF是否有影响
    }
}
