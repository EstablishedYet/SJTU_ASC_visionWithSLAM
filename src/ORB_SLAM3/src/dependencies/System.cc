...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename,bool createNewDelay)
{
    ...
    if (mSensor == System::IMU_MONOCULAR)
        for(size_t i_imu = 0; i_imu < vImuMeas.size(); i_imu++)
            mpTracker->GrabImuData(vImuMeas[i_imu]);
///--------------------------------------
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename,createNewDelay);
...
Sophus::SE3f System::RelocalizeMonocular(const cv::Mat &im,const double &timestamp,string filename,bool &flag)
{
    cv::Mat imToFeed = im.clone();
    if(settings_ && settings_->needToResize()){
        cv::Mat resizedIm;
    ...
    return mpTracker->GrabImageRelocalize(imToFeed,timestamp,filename,flag);
}
...
bool System::getResetActiveMapFlag()
{
    unique_lock<mutex> lock(mMutexReset);
...
void System::SaveFrame(int circle)
{
    if(circle==1)
    {
